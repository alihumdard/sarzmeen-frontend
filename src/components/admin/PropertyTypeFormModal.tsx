...
type PropertyTypeFormModalProps = {
  open: boolean;
  onClose: () => void;
  /** Passed when editing; omitted when creating. */
  propertyType?: PropertyTypeRecord | null;
  /** Categories a type can be filed under. */
  categories: Category[];
  onSubmit: (input: PropertyTypeInput) => Promise<void>;
};

const emptyForm: PropertyTypeInput = {
  name: "",
  slug: "",
  description: "",
  category: "",
...
export default function PropertyTypeFormModal({
  open,
  onClose,
  propertyType,
  categories,
  onSubmit,
}: PropertyTypeFormModalProps) {
  const isEdit = Boolean(propertyType);
  ...
  const [form, setForm] = useState<PropertyTypeInput>(
    propertyType
      ? {
          ...
          featured: propertyType.featured,
        }
      : emptyForm,
  );
  ...
  const [slugTouched, setSlugTouched] = useState(Boolean(propertyType));

  const update = <K extends keyof PropertyTypeInput>(
    key: K,
    value: PropertyTypeInput[K],
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (!form.category) {
      setError("Please choose a category for this type.");
      return;
    }
    ...
    try {
      ...
      setError("Could not save the property type. Please try again.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <AdminModal
      open={open}
      onClose={onClose}
      title={isEdit ? "Edit Property Type" : "Add New Property Type"}
      description={
        isEdit
          ? "Update this property type's details."
          : "Create a type that listings inside a category can be filed under."
      }
      size="md"
      footer={
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Field label="Status">
            <select
              value={form.status}
              onChange={(event) =>
                update("status", event.target.value as PropertyTypeStatus)
              }
              className={adminSelectClass}
            >
        ...
        <Field label="Description">
          <textarea
            ...
            placeholder="What kind of listings use this type?"
        ...
        <label className="flex cursor-pointer items-start gap-2.5">
          ...
          <span className="text-[12px] leading-relaxed text-gray-600">
            ...
            <span className="block text-[11px] text-gray-400">
              Featured types appear first in search filters.
            </span>
          </span>
        </label>
      </form>
